      ...
      <DialogContent className="sm:max-w-[500px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              {t('bulkImportDesc')} <code className="bg-muted px-1 py-0.5 rounded text-xs">{t('bulkImportFormat')}</code>
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            ...
            {text.trim() && (
              <div className="mt-4 text-sm">
                <p className="font-semibold mb-2">{t('previewTitle', { count: parsedItems.length })}</p>
                <div className="max-h-32 overflow-y-auto space-y-1 bg-muted/50 p-2 rounded border">
                  {parsedItems.length === 0 ? (
                    <span className="text-muted-foreground">{t('noMatchingItemsFound')}</span>
                  ) : (
                    parsedItems.map((item, i) => (
                      <div key={i} className="flex justify-between border-b last:border-0 pb-1 last:pb-0 border-border/50">
                        <span>{item.title}</span>
                        {item.dueDay && <span className="text-muted-foreground">{t('day')} {item.dueDay}</span>}
                      </div>
                    ))
                  )}
                </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={parsedItems.length === 0}>
              {t('importNItems', { count: parsedItems.length })}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
